...
public class SortController extends BaseController  {
    ...
    */
    ...
    public String index(Model model,@RequestParam(value = "pageNo", required = false, defaultValue = "1") Integer pageNo,@RequestParam(value = "pageSize", required = false, defaultValue = "20") Integer pageSize, String creatdateSpace, Sort sort) {
        ...
        if(!ObjectUtils.isEmpty(sort.getFenlei())) {
            queryWrapper = queryWrapper.like("fenlei",sort.getFenlei());
         }
        if(!ObjectUtils.isEmpty(sort.getAddr())) {
            queryWrapper = queryWrapper.like("addr",sort.getAddr());
         }
        if(!ObjectUtils.isEmpty(sort.getCreatdate())) {
            queryWrapper = queryWrapper.like("creatdate",sort.getCreatdate());
         }
        IPage<Sort> pageInfo = sortService.page(page, queryWrapper);
...
//        tit.getInfo(urlTwo,tiaojian1,nameUrl2,zbjService);
            ...
            for(int i = 0; i < 2; i++) {
                ...
                if() {
                String urlOne = "https;//" + nameUrl2 + ".zbj.com/search/p/k" + page + ".html?type=new&kw=" + nameUrl + "&d=" + nameUrl3;
                ReptileUtil tit = new ReptileUtil();
                tit.getInfo(urlOne, tiaojian1, nameUrl2, zbjService);
                page = 40 * i;
            } else {
                    //导出excel
